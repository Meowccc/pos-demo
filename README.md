...
- **備用方案**: localStorage (當 BroadcastChannel 不支援時)
...
### 備用方案
- 當 BroadcastChannel 不支援時，自動切換到 localStorage
- 定期檢查 localStorage 中的更新
- 使用 `pos_currentOrder` 作為儲存鍵值
...
2. 某些舊版瀏覽器可能不支援 BroadcastChannel，系統會自動使用備用方案
3. 建議使用現代瀏覽器以獲得最佳體驗
4. 確保兩個顯示器使用相同的域名和端口
